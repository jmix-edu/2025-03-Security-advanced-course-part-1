...
public class RegistrationCleaner implements Job {
    ...
    private final DateTimeTransformations dateTimeTransformations;
    private final SystemAuthenticator systemAuthenticator;

    public RegistrationCleaner(UnconstrainedDataManager unconstrainedDataManager,
    ...
    public String deleteOldNotActivatedUsers() {
        ...
        List<User> oldUsers = unconstrainedDataManager.load(User.class)
                .query("select u from User u where u.createdDate < :threshold and u.needsActivation = true")
                .parameter("threshold", threshold)
                .list();
        for (User u: oldUsers) {
            unconstrainedDataManager.remove(u);
    ...
    public void execute(JobExecutionContext jobExecutionContext) throws JobExecutionException {
        String result = deleteOldNotActivatedUsers();
